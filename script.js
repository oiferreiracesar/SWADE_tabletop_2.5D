...
function renderCell(row, col, fIndex, isGhost, activeEraseMode = false, applyCutaway = false, showActiveTools = false) {
    ...
    // O RETORNO TRIUNFAL DA PIRÂMIDE SÓLIDA (com o seu recorte diagonal genial)
    if (!isCutaway && fIndex >= 0 && !hideLowerRoof && enclosedCache[`${fIndex},${row},${col}`] && !hasStructureAbove(fIndex, row, col)) {
        ...
        // A MÁSCARA INTELIGENTE DO CHÃO: O segredo para domar as diagonais sem estragar o 3D!
        ctx.save();
        ...
        // Os painéis robustos da pirâmide inteiriça! Sem falhas, sem fragmentos.
